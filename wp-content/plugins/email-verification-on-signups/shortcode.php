...
class DWEmailVerifyShortcode{
	...
	function __construct(){
		...
		add_action( 'wp', array($this, 'verify_stuff') );
	}
	...
	public function verify_stuff(){
		...
		if( empty( $_GET['verify_email'] ) || empty( $_GET['user_id'] ) || ! preg_match( '/^[a-f0-9]{32}$/', $_GET['verify_email'] ) || ! DWEmailVerify::instance()->needs_validation( $user_id ) ) {
			...
			add_action( 'wp_head', array($this, 'page_redirect') );
			return;
		}
	...
	function shortcode_handler( $atts, $content = null ) {
		...
		switch ( $this->validation_status ) {
			case 'invalid_request':
				$output .= sprintf( __('Thanks! Your account has now been verified, if you are not redirected please <a href="%s">click here</a>.', 'dwverify'), DWEmailVerify::instance()->redirect_url() );
				break;
